...
3. Set global jealousy event chance to 100%, set the target character's jealousy appearance weight to 100%, and set the window to at least 10 minutes.
...
9. Set `reunionThresholdDays` low enough to match an old chat, or use the manual reunion test button.
...
- Anger-ready characters take priority over birthday / special-date mode.
- Birthday / special-date mode runs before jealousy and the normal reunion / primary draw when no anger-ready character exists.
