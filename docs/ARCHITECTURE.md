...
`maybeShowHomepageInvitation()` gives anger-ready characters the first slot, then checks date events and contextual callbacks:

```text
resolvePoolCharacters()
  ├─ if any anger-ready character exists
  │     → pick from anger candidates, mode = anger
  ├─ else if any birthday / special-date candidate exists
...
Reunion checks only the character that was already randomly drawn. It reads `/api/characters/chats` first and uses the maximum last-message timestamp returned in `last_mes` / `send_date`; this is preferred over SillyTavern `date_last_chat` because character stats can be based on chat file modification time after imports or syncs. Top-level `date_last_chat` is only a fallback when no chat message timestamp is available. Failed or legacy last-chat cache entries are ignored, so a transient API or parsing miss does not keep blocking reunion mode. This does not increase old characters' draw probability.
